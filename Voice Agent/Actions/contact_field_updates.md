...
| 12 | Save Order Status | `last_order_status` | After call | Replace | Whether the caller successfully placed an order, booked a reservation, only asked for information, or was transferred to a human during this call.  If the caller hung up, said nothing, or the call ended without any action being completed, set this field to: incomplete call. | `Order Confirmed` `Reservation Booked` `Info Only` `Transferred`,` incomplete call` |
